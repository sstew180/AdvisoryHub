...
export default function LibraryPage({ session }) {
  ...
  const loadDocs = () => {
    ...
    axios.get(API + '/api/library')
      ...
      .catch(err => { setError('Could not load library. The server may be starting up -- try again in 30 seconds.'); setLoading(false); });
  };
  ...
  return (
    <div className='page'>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
        ...
        {error && (
          <button className='btn btn-secondary' style={{ fontSize: 12 }} onClick={loadDocs}>
            Retry
          </button>
        )}
      </div>
      ...
      {error && !loading && (
        <div style={{ padding: '16px', background: 'var(--surface)', borderRadius: 'var(--radius)',
